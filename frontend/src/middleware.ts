...
export function middleware(req: NextRequest) {
  ...
  if (currentUserToken && Object.values(AUTH_ROUTE).includes(req.nextUrl.pathname)) {
    return NextResponse.redirect(new URL(PROTECTED_ROUTE.dashboard, req.url))
  }

  if (!currentUserToken && Object.values(PROTECTED_ROUTE).includes(req.nextUrl.pathname)) {
